...
The computational power of mobile devices has highly increased in the last few years and almost every device now comes with a GPS and compass sensor. This opens up possibilities to enhance the user experience in daily life. In this paper an application which has a 3-D audio engine as its core is presented which makes use of head related transfer function (HRTF) model to improve and augment the auditory experience of the user in addition to provide her or him with natural navigational cues. In a small experiment the developed 3-D audio engine is compared with a cosine panner modell~\cite{AndyFarnell2010} in terms of quality and efficiency of the navigational cues. Although the experiment did not reveal significant differences between the two models a critical observation of this study supports that a more sophisticated 3-D audio engine can increase the user experience in audio navigation.
\end{abstract}
...
Although it is a recent field of research, several works and experiments in the context of audio augmented reality has been made so far. Most of research has been done mainly on the use of visual information with very little attention paid to the audio aspects. In an augmented reality environment, using audio cues is a relevant concern and an audio environment involves many issues and challenges~\cite{rozier2000}. For instance, providing a suitable way for the user to orient himself or herself in an only-audio virtual space is even more complicated than providing visual cues. Contrary to visual images, audio is in fact unstable and thus need to be continuously repeated or be infinite in order to work as a directional cue. As pointed in Rozier~\cite{rozier2000}, one peculiar aspect in audio is that as soon is heard, it starts disappearing and it can never be examined as a whole. New paradigm and strategies in listening to audio must be thought to let the listener glean whatever useful sound details. (MG: I think this is not so well explained).   
...
It was necessary to isolate the output $Y(z)$ in Equation~\ref{eq:filterparam} as well as going from the frequency to the time domain. The result is given in Equation~\ref{eq:out}\footnote{For the derivation of Equation~\ref{eq:out} see appendix A}:
